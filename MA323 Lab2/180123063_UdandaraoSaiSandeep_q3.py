...
def gen(num,case):
    ...
    cn = 0
    for v in arr:
        arr[cn] = (0.5)*(1-math.cos(v*(math.pi)))
        s += arr[cn] 
        cn+=1
    mean = s/num
    maxx = -1.0
...
y1 = []
for var in x1:
    y1.append((2/(math.pi))*(math.asin(math.sqrt(var))))
graph.plot(x1,y1,'r')
graph.xlabel('X-axis')
graph.ylabel('F(x)')
graph.title('Cumulative Distribution Function')
graph.show()
print('Actual Value of Mean:',0.5)
# Actual mean Calulated through Integration Definition of Expectation
print('Actual Value of Variance:',0.125)
# Actual Variance Calulated through Integration Definition of Variance